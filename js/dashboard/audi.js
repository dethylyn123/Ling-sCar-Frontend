import {
  ...
  async function getData() {
    ...
    const response = await fetch(backendURL + "/api/audicar/all", {
      headers: {
...
container += `
  ...
  form_users.onsubmit = async (e) => {
    ...
    if (for_update_id == "") {
      ...
      response = await fetch(backendURL + "/api/audicar/store", {
        method: "POST",
        headers: {
          Accept: "application/json",
          Authorization: "Bearer " + localStorage.getItem("token"),
  ...
  const deleteAction = async (e) => {
    ...
    if (confirm("Are you sure you want to delete?")) {
      ...
      const response = await fetch(backendURL + "/api/audicar/destroy/" + id, {
        method: "DELETE",
        headers: {
          Accept: "application/json",
          Authorization: "Bearer " + localStorage.getItem("token"),
  ...
  const showData = async (id) => {
    ...
    const response = await fetch(backendURL + "/api/audicar/show/" + id, {
      headers: {
        Accept: "application/json",
        Authorization: "Bearer " + localStorage.getItem("token"),
        "ngrok-skip-browser-warning": "69420", // Include ngrok bypass header directly
